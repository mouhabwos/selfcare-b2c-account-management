...
class AbonneResourceIntTest {
    ...
    @Test
    void isPostpaid() throws Exception {
        ...
        restAbonneMockMvc
            .perform(get("/api/abonne/is-postpaid/{msisdn1}/{msisdn}", DEFAULT_NUMERO, DEFAULT_NUMERO_TO_VERIFIER))
    ...
    void getRequestStatus() throws Exception {
        ...
        restAbonneMockMvc.perform(get("/api/abonne/request/{requestId}", 51022830)).andExpect(status().isOk());
    }

    @Test
    void getCustomerOffer() throws Exception {
        ...
        restAbonneMockMvc.perform(get("/api/abonne/v1/customerOffer/{msisdn}", DEFAULT_NUMERO)).andExpect(status().isOk());
    }

    @Test
    void testGetOrganizationInformationInformation() throws Exception {
        ...
        restAbonneMockMvc
            .perform(get("/api/abonne/v1/is-orange-number/{msisdn}", "DEFAULT_NUMERO"))
            .andExpect(status().isOk())
    ...
    void testGetClientInfo() throws Exception {
        ...
        restAbonneMockMvc.perform(get("/api/abonne/infos-client/{msisdn}", "DEFAULT_NUMERO")).andExpect(status().isOk());
    }

    @Test
    void getCustomerOfferWithoutClientCode() throws Exception {
        CustomerOffer customerOffer = new CustomerOffer();
        customerOffer.setClientCode("0012707812");
        customerOffer.setCreateDate("2011-05-26T15:51:10");
        customerOffer.setEndUserId("771326617");
        ...
        restAbonneMockMvc
            .perform(get("/api/abonne/v2/customerOffer/{msisdn}", DEFAULT_NUMERO))
            .andExpect(status().isOk())
    ...
    @Test
    void testIsOrganizationNumber() throws Exception {
        restAbonneMockMvc
            .perform(get("/api/abonne/v1/is-coorporate-number/{msisdn}", "DEFAULT_NUMERO"))
            .andExpect(status().isOk())
            .andExpect(content().string("false"));
    }

    @Test
    void testGetMyContactNumbers() throws Exception {
        restAbonneMockMvc.perform(get("/api/abonne/v1/contact-numbers/{msisdn}", "782363572")).andExpect(status().isOk());
    }

    @Test
    void testGetNumberStatus() throws Exception {
        when(abonneService.getNumberStatus(anyString())).thenReturn(ResponseEntity.ok("ACTIVATED"));
        restAbonneMockMvc
            .perform(get("/api/abonne/v1/number/{msisdn}/status", "338328033"))
            .andExpect(status().isOk())
            .andExpect(content().string("ACTIVATED"));
    }

    @Test
    void testGetNumberStatusWithError() throws Exception {
