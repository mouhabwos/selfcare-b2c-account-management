...
@IntegrationTest
class ValidationHmacServiceTest {
    ...
    @Test
    void testCheckHmacInvalid() {
        ValidationHmacService.InvalidHmacException thrown = org.junit.jupiter.api.Assertions.assertThrows(
            ValidationHmacService.InvalidHmacException.class,
            () -> {
    ...
    @Test
    void testCheckHmacValid() {
        String hmac = SHA256Handler.encryptSHA256(
            String.format(
                "%s%s%s%s",
                INDICATIF + MSISDN,
                UUID,
                new SimpleDateFormat("dd/MM/yyyy").format(NOW),
                applicationProperties.getHmacSecret()
            )
        );
