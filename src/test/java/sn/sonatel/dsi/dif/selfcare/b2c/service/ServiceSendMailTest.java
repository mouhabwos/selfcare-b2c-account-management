...
public class ServiceSendMailTest {
    ...
    @Autowired
    private MailSendRepository mockMailSendRepository;

    private ServiceSendMail serviceSendMailUnderTest;

    @Before
    public void setUp() {
        initMocks(this);
        serviceSendMailUnderTest = new ServiceSendMail(mockJHipsterProperties, mockJavaMailSender, mockTemplateEngine, mockApplicationProperties, mockMailSendRepository);
    }
    ...
    private Map<String, DataSource> getDataSourceMap() throws Exception {
        ...
        DataSource dsFormulaire = new ByteArrayDataSource(multipartFilePDF.getBytes(), multipartFilePDF.getContentType());
        DataSource dsRecto = new ByteArrayDataSource(multipartFileRecto.getBytes(), multipartFileRecto.getContentType());
        DataSource dsVerso = new ByteArrayDataSource(multipartFileVerso.getBytes(), multipartFileVerso.getContentType());

        dataSource.put(filePdf,dsFormulaire);
        dataSource.put(image,dsRecto);
        dataSource.put(image,dsVerso);
        return dataSource;
    }
    ...
    public void testSendEmail() throws Exception {
        // Setup
        final Map<String, DataSource> sourceList = getDataSourceMap();
        final OperationDTO user = operationDTO();

        // Run the test
        serviceSendMailUnderTest.sendEmailWithAttachement(sourceList,"test","Content", true, false, getMailEntity());
