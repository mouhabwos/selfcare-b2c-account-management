...
public class DeleteAccountService {
    ...
    private void deleteRattachementLigne(String msisdn) {
        ...
        numeroRattache.ifPresent(rattachementLigne -> {
            ...
            smsNotificationService.sendSMSPP(msisdn,applicationProperties.getSendSms().getAccountDeletion().getSmsContent(), Constants.ORANGE_ET_MOI);
        });
    }
    ...
    private void deleteLigneRattacheForPrincipal(AccountB2C principal){
        ...
        allByAccountB2C.forEach(rattachementLigne -> {
            rattachementLigneRepository.deleteById(rattachementLigne.getId());
            smsNotificationService.sendSMSPP(rattachementLigne.getNumero(),applicationProperties.getSendSms().getAccountDeletion().getSmsContent(), Constants.ORANGE_ET_MOI);
        });
    }
