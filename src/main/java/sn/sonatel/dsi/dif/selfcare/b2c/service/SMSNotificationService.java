...
public class SMSNotificationService {
    ...
    public boolean sendSMSPP(String msisdn, String message,String sourceAddress) {
        ...
        try {
            ...
            session.connectAndBind(applicationProperties.getSendSms().getServer().getSmsHost(), applicationProperties.getSendSms().getServer().getSmsPort(),

                new BindParameter(

                    BindType.BIND_TX, applicationProperties.getSendSms().getServer().getSmsSystemId(), applicationProperties.getSendSms().getServer().getSmsPassword(),

                    applicationProperties.getSendSms().getServer().getSmsSystemType(), TypeOfNumber.UNKNOWN, NumberingPlanIndicator.UNKNOWN, null));

            session.submitShortMessage("CMT", TypeOfNumber.ALPHANUMERIC, NumberingPlanIndicator.UNKNOWN,

                sourceAddress, TypeOfNumber.NATIONAL, NumberingPlanIndicator.UNKNOWN,
